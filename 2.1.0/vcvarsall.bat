...
@REM The known case this affects is the Platform environment variable, which
...
echo where :
echo     [arch]: x64 ^| x86 ^| arm64 ^| x64_x86 ^| x64_arm64 ^| x86_x64 ^| x86_arm64 ^| arm64_x64 ^| arm64_x86
...
echo:
echo For the host and target architectures, 'amd64' is supported as an alias for 'x64'.
echo:
echo The 'store' parameter sets environment variables to support Universal Windows Platform
echo application development and is an alias for 'uwp'.
echo:
echo For example:
echo     %~nx0 x86_amd64
